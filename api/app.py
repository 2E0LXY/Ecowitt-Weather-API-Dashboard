...
async def call_gemini_forecast(context_payload, satellite_ai_images=None):
    ...
    prompt = (
        ...
        "If thunderstorm_risk data is present, incorporate it prominently: CAPE (J/kg) measures atmospheric "
        "instability (>1000 = significant, >2000 = severe), CIN measures convective inhibition (near 0 = storms "
        "fire freely, strongly negative = capped). Cross-reference CAPE with the WH57 sensor actual strike count. "
        "CRITICAL RULE: If nearby_lightning is true OR wh57_strikes_today > 0, you MUST begin the forecast "
        "section with a clear, prominent thunderstorm warning as the very first sentence. Do not bury it. "
        "The WH57 sensor is ground truth - if it detects strikes, storms ARE active regardless of CAPE value. "
        "If the model CAPE is low but real strikes are detected, explicitly note the model is lagging reality. "
        "High CAPE with CIN near zero but no strikes = imminent risk, flag clearly. "
        "High CAPE with strong negative CIN = potential but suppressed for now.\n"
